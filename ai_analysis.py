...
# GEMINI MAX INLINE FILE BYTES
GEMINI_MAX_INLINE_FILE_BYTES = 7 * 1024 * 1024
...
def generate_ai_analysis(
    ...
    # build the contents list with the text prompt; track payload size so the
    # inline request stays under Gemini's 20 MB limit
    contents = [full_prompt]
    image_count = 0
    skipped_oversize_files = 0
    inline_payload_bytes = len(full_prompt.encode("utf-8"))
    ...
    if temp_image_dir and os.path.isdir(temp_image_dir):
        ...
        for filename in sorted(os.listdir(temp_image_dir)):
            ...
            # check size before reading so oversized files never enter the payload
            file_size = os.path.getsize(file_path)
            if file_size > GEMINI_MAX_INLINE_FILE_BYTES:
                skipped_oversize_files += 1
                print(
                    f"  Skipping {filename} ({file_size / (1024 * 1024):.1f} MB); "
                    f"exceeds Gemini's {GEMINI_MAX_INLINE_FILE_BYTES // (1024 * 1024)} MB "
                    f"per-file inline limit"
                )
                continue

            if inline_payload_bytes + file_size > GEMINI_INLINE_REQUEST_SIZE_LIMIT_BYTES:
                print(
                    f"  Stopping image upload at {image_count} images "
                    f"({inline_payload_bytes / (1024 * 1024):.1f} MB); "
                    f"adding {filename} ({file_size / (1024 * 1024):.1f} MB) would "
                    f"exceed Gemini's {GEMINI_INLINE_REQUEST_SIZE_LIMIT_BYTES // (1024 * 1024)} MB "
                    f"per-request limit"
                )
                break
            ...
            inline_payload_bytes += len(image_bytes)
            image_part = types.Part.from_bytes(data=image_bytes, mime_type=mime_type)
            contents.append(image_part)
            image_count += 1

    if skipped_oversize_files:
        print(f"  Skipped {skipped_oversize_files} image(s) over the per-file size limit")
